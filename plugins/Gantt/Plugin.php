...
class Plugin extends Base
{
    public function initialize()
    {
        ...
        /* Only the pages that draw a chart need the chart code. TaskGrid is in
           the list because its Gantt mode renders into the same #gantt-chart
           container. An unreadable controller registers everything, so the
           failure mode is a slightly heavier page, never a blank chart. */
        if ($this->isGanttPage()) {
    ...
    /**
     * Whether the page being rendered draws a Gantt chart.
     *
     * @return boolean
     */
