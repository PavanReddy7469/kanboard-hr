...
class ProjectCreationController extends BaseController
{
    ...
    public function create(array $values = array(), array $errors = array())
    {
        ...
        $this->response->html($this->helper->layout->app('project_creation/create', array(
            ...
            'users_list' => array(0 => t('Unassigned')) + $this->userModel->getActiveUsersList(),
    ...
    {
        /* Description, the dates and the owner are on the creation form but were
           not in this list, so they were collected and thrown away - a project
           created with an end date came out with none. task_limit was read
           without a guard and the form does not send it, which raised an
           undefined-key warning on every creation. */
        $project = array(
            'name' => $values['name'],
            ...
            'task_limit' => array_key_exists('task_limit', $values) ? $values['task_limit'] : 0,
        );

        // Honoured by TaskManager's ProjectModel after creation; core's create()
        // always stamps owner_id with the creating user.
        if (! empty($values['owner_id'])) {
            $project['owner_id'] = (int) $values['owner_id'];
        }
