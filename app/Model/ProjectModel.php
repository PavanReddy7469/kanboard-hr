...
class ProjectModel extends Base
{
    ...
    public function create(array $values, $userId = 0, $addUser = false)
    {
        ...
        if (! empty($values['identifier'])) {
            $values['identifier'] = strtoupper(substr($values['identifier'], 0, 4));
        } else {
            $values['identifier'] = $this->generateUniqueIdentifier();
        }
    ...
    /**
     * Generate a unique 4-character alphanumeric project identifier.
     *
     * @return string
     */
    public function generateUniqueIdentifier()
    {
        ...
        for ($i = 0; $i < 100; $i++) {
            ...
            $exists = $this->db->table(self::TABLE)->eq('identifier', $code)->exists();
            if (! $exists) {
                return $code;
            }
        }
