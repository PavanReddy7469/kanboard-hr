...
class TaskCreationModel extends Base
{
    ...
    protected function prepare(array &$values)
    {
        ...
        if (empty($values['reference'])) {
            $values['reference'] = $this->generateUniqueTaskCode();
        } else {
            $values['reference'] = strtoupper(substr($values['reference'], 0, 4));
        }
    ...
    /**
     * Generate a unique 4-character alphanumeric task code.
     *
     * @return string
     */
    public function generateUniqueTaskCode()
    {
        ...
        for ($i = 0; $i < 100; $i++) {
            ...
            $exists = $this->db->table(TaskModel::TABLE)->eq('reference', $code)->exists();
            if (! $exists) {
                return $code;
            }
        }
