...
CREATE TABLE IF NOT EXISTS `taskmanager_deliverables` (
  ...
  PRIMARY KEY (`id`),
  KEY `taskmanager_deliverables_task_idx` (`task_id`),
  KEY `taskmanager_deliverables_project_idx` (`project_id`, `status`),
  CONSTRAINT `taskmanager_deliverables_ibfk_1` FOREIGN KEY (`task_id`) REFERENCES `tasks` (`id`) ON DELETE CASCADE,
  CONSTRAINT `taskmanager_deliverables_ibfk_2` FOREIGN KEY (`project_id`) REFERENCES `projects` (`id`) ON DELETE CASCADE,
  CONSTRAINT `taskmanager_deliverables_ibfk_3` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
